...
async fn main() -> Result<()> {
    ...
    ui.on_forgot_password({
        move || {
            ...
            slint::spawn_local(async move {
                let result = forgot_password(&conn_clone, &username, black_square_window_handle.clone()).await;
...
async fn register_user(conn: &Arc<Mutex<Connection>>, username: &str, password: &str) -> Result<()> {
    let hashed_password = hash_password(password)?;
    let random_hash = generate_random_hash()?;

    if let Some(file_path) = FileDialog::new().set_file_name("masterkey.txt").save_file() {
        ...
        Err(anyhow!("Registration failed: User cancelled file save"))
    }
}
